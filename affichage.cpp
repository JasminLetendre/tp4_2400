...
void tracerLigne(vector<vector<std::string>> &grille, int x0, int y0, int x1,
  ...
  if (lignes == 0) {
    ...
    for (int i = 0; i <= lignes; ++i) {
      // On trace chaque point de la ligne, de gauche à droite ou de droite à
      // gauche selon la direction.
...
void imprimerGrille(const vector<Point> &points) {
  // On crée une grille.
  vector<vector<std::string>> grille(HAUTEUR,
                                     vector<std::string>(LARGEUR, " "));
